var config_obj={
    ...
    other_config:function(){
        ...
        $("input[name=Dis_Agent_Type]").click(function(){
            ...
            var regwhere_procitaget ='';
            var regwhere_citaget ='';
            var regwhere_couaget ='';
            var type = $(this).attr('value');
            if(type == 0){
                $("#Agent_Rate_Row").hide();
                $("#Agent_Ratpc_Row").hide();
                $("#Agent_Rata_Row").hide();
                $("#Agent_Ratb_Row").hide();
                $("#Agent_Ratc_Row").hide();
                $("#Agent_Ratd_Row").hide();
                // $("#Agent_Rate_Input").html('');
                // $("#Agent_Ratpc_Input").html('');
                ...
                //省	会
                regwhere_procitaget += "利润率%<input type=\"text\" name=\"Agent_Rate[procit][Province]\" value=\"0\" class=\"form_input\" size=\"3\" maxlength=\"10\" notnull>&nbsp;&nbsp;&nbsp;代理价格<strong class=\"red\">（元）</strong><input type=\"text\" name=\"Agent_Rate[procit][Provincepro]\" value=\"0\" class=\"form_input\" size=\"3\" maxlength=\"10\" notnull >认证条件：";
                if(level != null){
                    regwhere_procitaget += "等级<select name=\"Agent_Rate[procit][Level]\"><option value=\"0\" selected>---选择等级---</option>";
                    $.each(level,function(key,val){
                        regwhere_procitaget += "<option value=\""+val['Level_ID']+"\">"+val['Level_Name']+"</option>";
                    });
                    regwhere_procitaget += "</select>";
                }
                if(title != null){
                    regwhere_procitaget += "&nbsp;&nbsp;&nbsp;爵位<select name=\"Agent_Rate[procit][Protitle]\"><option value=\"0\" selected>---选择爵位---</option>";
                    $.each(title,function(key,val){
                        regwhere_procitaget += "<option value=\""+key+"\">"+val['Name']+"</option>";
                    });
                    regwhere_procitaget += "</select>";
                }
                regwhere_procitaget += "&nbsp;&nbsp;&nbsp;自费金额<strong class=\"red\">（元）</strong><input type=\"text\" name=\"Agent_Rate[procit][Selfpro]\" value=\"0\" class=\"form_input\" size=\"3\" maxlength=\"10\" notnull>&nbsp;&nbsp;&nbsp;团队销售额<strong class=\"red\">（元）</strong><input type=\"text\" name=\"Agent_Rate[procit][Teampro]\" value=\"0\" class=\"form_input\" size=\"3\" maxlength=\"10\" notnull>";
                ...
                $("#Agent_Rate_Row").show();
                $("#Agent_Ratpc_Row").show();
                $("#Agent_Rata_Row").show();
                $("#Agent_Ratb_Row").show();
                $("#Agent_Ratc_Row").show();
                $("#Agent_Ratd_Row").show();
                // $("#Agent_Rate_Input").html(regwhere_proaget);
                // $("#Agent_Ratpc_Input").html(regwhere_procitaget);
