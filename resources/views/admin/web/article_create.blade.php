        ...
        <div id="iframe_page">
            <div class="iframe_content">
                ...
                <div id="column" class="r_con_wrap">
                    <form class="r_con_form" method="post" action="/admin/web/article_store" id="article_form">
                        {{csrf_field()}}
                        <div class="rows">
                            <label>排序</label>
                            <span class="input">
                                <input name="Index" value="{{old('Index')}}" type="number" class="form_input" size="10" required>
                                越大越靠后
                            </span>
                            <div class="clear"></div>
                        </div>
                        <div class="rows">
                            <label>内容标题</label>
                            <span class="input">
                                <input name="Title" value="{{old('Title')}}" type="text" class="form_input" size="40" maxlength="50" required>
                                ...
                                <select name="Column_ID" required>
                                    @foreach($Columns as $Column){
                                          ...
                                          @foreach($Column['child'] as $item)
                                              <option value="{{$item['Column_ID']}}" @if(old('Column_ID') == $item['Column_ID']) selected @endif> └ {{$item['Column_Name']}}</option>
                                          @endforeach
                                    @endforeach
                                </select>
                                <span class="fc_red">*</span>
                        ...
                        <div class="rows" id="BriefDescription_rows">
                            <label>简短介绍</label>
                            <span class="input">
                                <textarea class="txetarea" name="BriefDescription">{{old('BriefDescription')}}</textarea>
                            </span>
                            <div class="clear"></div>
                        </div>
                        <div class="rows" id="Description_rows">
                            <label>详细内容</label>
                            <span class="input">
                                <textarea name="Description" style="width:100%;height:400px;visibility:hidden;">{{old('Description')}}</textarea>
                            </span>
                            <div class="clear"></div>
                        </div>
                        <div class="rows">
