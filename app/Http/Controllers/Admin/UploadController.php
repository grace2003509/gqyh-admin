...
class UploadController extends Controller
{
    ...
    {
        include('/admin/js/kindeditor/php/JSON.php');

        /*自定义传入参数开始*/
        $UploadFiles_TableField = $request->get('TableField');
        $dir = $request->get('dir');
        ...
        if (empty($_FILES) === false) {
            ...
            if ($file_size > $max_size) {
                $this->alert("上传文件大小超过" . ($max_size / 1024) . "K限制。");
            }
            //检查目录名
...
/*                $thumImg->littleImage($file_path, $save_path . 'n1/', 300);
            ...
            echo $json->encode(
                array(
                    'error' => 0,
                    'url' => $file_url,
                    'size' => number_format($file_size / 1024, 2, ".", ""))
            );
            exit;
        }
    }
